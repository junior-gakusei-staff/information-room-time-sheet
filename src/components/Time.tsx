...
const Timetable: React.FC<TimetableProps> = async ({
  roomName,
  currentDay,
}) => {
  ...
  const currentDayName =
    Object.keys(dayMapping).find((key) => dayMapping[key] === currentDayKey) ||
    "";

  const renderTableRows = (classPeriod: `class-${number}`, index: number) => {
    const classInfo = (timetable as WeekSchedule)[
      currentDayKey as keyof WeekSchedule
    ]?.[classPeriod];
  ...
  return (
    <div className="bg-gray-100 p-6 rounded-xl shadow-lg">
      ...
      <div className="space-y-4">
        ...
        ).map((classPeriod, index) => renderTableRows(classPeriod, index + 1))}
      </div>
    </div>
  );
};
