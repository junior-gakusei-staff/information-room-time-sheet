...
// T60* のみの配列
export const t60Rooms: string[] = roomName.filter((room) =>
  /^[tT]60\d+$/.test(room),
);

// T70* のみの配列
export const t70Rooms: string[] = roomName.filter((room) =>
  /^[tT]70\d+$/.test(room),
);

// E50* のみの配列
export const e50Rooms: string[] = roomName.filter((room) =>
  /^E50\d+$/.test(room),
);
...
const reverseDayMapping: ReverseDayMapping = Object.entries(dayMapping).reduce(
  (acc, [key, value]) => {
...
export function japaneseToEnglishDay(
  japaneseDay: JapaneseDayOfWeek,
): EnglishDayOfWeek {
...
export function englishToJapaneseDay(
  englishDay: EnglishDayOfWeek,
): JapaneseDayOfWeek {
...
export function transformWeek(
  day: JapaneseDayOfWeek | EnglishDayOfWeek,
): EnglishDayOfWeek | JapaneseDayOfWeek {
